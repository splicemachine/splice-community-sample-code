...
import com.splicemachine.tutorials.function.JsonFunction;
...
/**
 * This is an example of a VTI that reads in a list of JSON
 * and parses it and returns records for each entry found
 * 
 * @author Erin Driggers
 *
 */
public class ArrayListOfJsonVTI  implements DatasetProvider, VTICosting{
    
    /**
     * Log object to log messages.  The log messages in this class will appear
     * in the HBASE Region Server logs
     */    
    private static final Logger LOG = Logger.getLogger(ArrayListOfJsonVTI.class);
     ...
     */
    public ArrayListOfJsonVTI (Blob  pRecords) {
    ...
    public DataSet<LocatedRow> getDataSet(SpliceOperation op, DataSetProcessor dsp, ExecRow execRow) throws StandardException {
        ...
        //DataSet<String> textSet =  new SparkDataSet<>(SpliceSpark.getContext().parallelize(this.records));
        return textSet.flatMap(new JsonFunction(execRow, operationContext), true);
    }
