...
public class FeatureReport {
    ...
    //Columns in the return result set
...
//          new GenericColumnDescriptor("VARIANCE", DataTypeDescriptor.getBuiltInDataTypeDescriptor(Types.DOUBLE)),
            new GenericColumnDescriptor("STANDARD_DEVIATION", DataTypeDescriptor.getBuiltInDataTypeDescriptor(Types.DOUBLE)),
...
//          new GenericColumnDescriptor("NORML2", DataTypeDescriptor.getBuiltInDataTypeDescriptor(Types.DOUBLE)),
    };

    public static void continuousFeatureReport(String tableName, ResultSet[] resultSets) throws StandardException {
        try {
            ...
            if (numColumns > 0) {
                ...
                IteratorNoPutResultSet resultsToWrap = wrapResults((EmbedConnection) conn, getColumnStatistics(ps2, summary, fieldsToConvert));
     ...
     */
    private static Iterable<ExecRow> getColumnStatistics(PreparedStatement ps, MultivariateStatisticalSummary summary, int[] fieldsToConvert) throws StandardException {
        try {
            ...
            for (int columnPosition : fieldsToConvert) {
    ...
    private static IteratorNoPutResultSet wrapResults(EmbedConnection conn, Iterable<ExecRow> rows) throws StandardException {
        ...
        IteratorNoPutResultSet resultsToWrap = new IteratorNoPutResultSet(rows, STATEMENT_STATS_OUTPUT_COLUMNS, lastActivation);
