 ...
 */
public class DataFrameVTI implements DatasetProvider, VTICosting {
       
    /**
     * Log object to log messages.  The log messages in this class will appear
     * in the HBASE Region Server logs
     */    
    private static final Logger LOG = Logger.getLogger(DataFrameVTI.class);
    
    /**
     * This is the DataFrame that contains the source
     * data to be saved to Splice Machine.
     */
     ...
     * 
     */
    public DataFrameVTI(Blob  pDataFrame, ArrayList<String> dataFrameColumns) throws SQLException, ClassNotFoundException, IOException{
        if(pDataFrame != null) {
            if(pDataFrame != null) {
                this.rows = (java.util.List<Row>)getBlobResults(pDataFrame);
            }
        }
     ...
     * 
     */
    public static DatasetProvider getDataFrameVTI(Blob  pDataFrame, ArrayList<String> dataFrameColumns) throws SQLException, ClassNotFoundException, IOException {
        return new DataFrameVTI(pDataFrame,dataFrameColumns);
    }
    ...
    public DataSet<LocatedRow> getDataSet(SpliceOperation op, DataSetProcessor dsp, ExecRow execRow) throws StandardException {
        ...
        try {
            ...
            DataFrameIterator it = new DataFrameIterator(this.rows, this.dataFrameColumns, execRow);
