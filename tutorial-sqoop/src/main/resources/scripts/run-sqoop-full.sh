		...
		do
			...
			if [ -z "$query" ]; then
				if [ $column ]; then
					...
					sqoop import --options-file $SQOOP_CONFIG_FILE --append --table $TABLE --schema $schema --split-by $COLUMN --target-dir /data/$SPLICE_SCHEMA/$table --m $MAPPERS
				else
					echo Execute sqoop without split-by
					sqoop import --options-file $SQOOP_CONFIG_FILE --append --table $TABLE --schema $schema --target-dir /data/$SPLICE_SCHEMA/$table --m $MAPPERS
				fi
			else 
				if [ $column ]; then
					echo Execute sqoop with split-by
