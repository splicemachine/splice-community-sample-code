...
/**
 * This generates the ImpressionLog and puts them on Kafka Queue.
 *
...
public class kafkaStreamGenerator {
    ...
    public void generateMessages() {
        ...
        props.put("value.serializer", "com.splicemachine.tutorials.tsdbanalytics.dataobjects.ImpressionLogSerializer");

        // Create a KafkaProducer using the Kafka Connection properties
        KafkaProducer<String, ImpressionLog> producer = new KafkaProducer<String, ImpressionLog>(props);

        int i = 0;

        while ((totalEvents == 0) || (i < totalEvents)) {

            // Generate ImpressionLog
            long timestamp = System.currentTimeMillis();
            String publisher = "publishers_" + random.nextInt(this.numPublishers);
